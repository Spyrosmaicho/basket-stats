...
# Button to manually refresh from SQL
if st.sidebar.button("🔄 Sync with C Database"):
    st.cache_data.clear()
    st.rerun()
...
def load_data():
    ...
    if not os.path.exists(db_path):
        st.error(f"Database not found at {db_path}. Please run the C program first!")
        return pd.DataFrame()
    ...
    try:
        ...
        # Μετονομασία στηλών από C σε Python UI format
        df.rename(columns={
            'FT_Pct': '1pt %',
            'TwoP_Pct': '2pt %',
            'ThreeP_Pct': '3pt %'
        ...
        # True Shooting calculation (Approximated if Attempts aren't in DB)
        # We use Points as a proxy for Volume here
...
if df.empty:
    st.warning("Waiting for data... Ensure your C program has populated the database.")
    st.stop()
