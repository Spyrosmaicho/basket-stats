...
- **Smart Image Resolution Engine**: A multi-tiered, asynchronous image fetching system. It intelligently resolves player headshots by falling back from local cache -> NBA CDN -> Wikipedia Action API -> UI-Avatars, complete with User-Agent spoofing to bypass rate limits.
...
├── .github/
│   └── workflows/
│       └── c-build.yml              # CI/CD pipeline (GitHub Actions automated build testing)
│
├── analytics/                      # Python analytics + Streamlit frontend
│   ├── app.py                      # Streamlit entry point
│   ├── config.py                  # Global configuration
│   ├── styles.py                  # UI styling system
│   │
│   ├── engine/                    # Core processing & backend interface
│   │   ├── daemon.py              # Background processes / data sync
...
│   │   └── __init__.py
│   │
│   ├── matchup/              # Head-to-head analysis
│   │   ├── matchup.py
│   │   └── __init__.py
│   │
│   ├── queries/             # Advanced filtering / query system
│   │   ├── advanced_queries.py
│   │   └── __init__.py
│   │
...
│       ├── anonymous_player.png
│       ├── nba_logo.png
│       └── backgrounds/
│
├── database/               # Relational storage
│   └── nba_stats.db        # SQLite database generated by C backend
│
├── include/                # C headers (data structures & APIs)
│   ├── hashtable.h
│   ├── vector.h
│   ├── json_parser.h
│   └── ...
...
│   └── run.sh
│
├── src/                    # C source code (core engine)
│   ├── main.c
│   ├── hashtable.c
│   ├── vector.c
│   └── ...
│
├── tests/                  # Data ingestion & API scripts
│   ├── fetch_nba.py
│   ├── nba.json
│   └── new.txt
│
...
DASH[Dashboard & Analytics Layer]
CHARTS[Visualization Engine]
ML[ML / Intelligence Layer]

ENGINE[Python Engine Layer]
DB[(SQLite Database)]

C[C Backend Daemon Mode]
